...
const AnswerOptions = ({
  ...
  return (
    <div className="grid grid-cols-1 gap-4 mb-8 w-full max-w-2xl">
      {options.map((option, i) => {
        const isCorrect = option === correctAnswer;
        const isSelected = option === selected;
        const isRevealed = revealed && (isCorrect || isSelected);
        ...
        return (
          <motion.button
            ...
            className={`relative px-6 py-4 rounded-xl font-semibold text-lg transition-all duration-300 backdrop-blur-lg border ${
              isRevealed
                ? isCorrect
          ...
          >
            ...
            {isRevealed && (
              <span className="ml-3">{isCorrect ? '✅' : '❌'}</span>
            )}
          </motion.button>
        );
      })}
